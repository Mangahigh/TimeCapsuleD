...
module.exports = {
  start: (passedConfig) => {
    const config = configLib.getConfig(passedConfig);
    ...
    const keyLib = new KeyManager(config);
    const queueLib = new Queues(keyLib);
    const redisLib = new Redis(config);
    const serverLib = new Server(config);
    ...
    Promise.all(initialConnections).then((initialConnections) => {
      const [server, redisClientPublish, redisClientInpender, redisClientStats] = initialConnections;

      const inpender = new Inpender(keyLib, queueLib, redisClientInpender, config);
