    ...
    sock.on('data', (data) => {
      switch (data.toString().split(' ')[0].trim()) {
        ...
        case ACTION.FETCH:
          ...
          this._getNextAndSend(sock, data).then((returnedGetter) => {
  ...
  getStats() {
    return new Promise((resolve) => {
      this._redisLib.connect().then((redisClient) => {
        ...
        this._queueLib.getAll(redisClient).then((queues) => {
          stats.push('__queueCount: ' + queues.length);
          return statsLib.getStats(queues);
        }).then((data) => {
