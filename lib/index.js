...
class TimeCapsule {
  ...
  _cleanUpUnusedRedisClients() {
    ...
    // while (redisClient = (this._redisClientSubscriberPool.length > this._config.minRedisClientSubscriberPool && this._redisClientSubscriberPool.pop())) {
    //   redisClient.quit();
    // }
  }
  ...
  runServer() {
    ...
    Promise.all(initialConnections).then((connections) => {
      ...
      setInterval(() => {
        this._logger.info('Cleaning up unused');
        this._cleanUpUnusedRedisClients();
      }, 5 * 1000);
