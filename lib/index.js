...
module.exports = {
  ...
  start: function (config) {
    ...
    function getNextEvent(queue, callback) {
      ...
      client.on('connect', function () {
        client.blpop([redisNamespace, queue, 'list'].join('.'), 0, function (err, data) {
          client.hgetall([redisNamespace, queue, 'data'].join('.') + ':' + data[1], function (err, data) {
            client.quit();
     ...
     * @param {Function} successCallback
     */
    function getFutureCount(queueName, successCallback) {
      client.zcount([redisNamespace, queueName, 'index'].join('.'), '-inf', '+inf', function (err, count) {
        successCallback(queueName, 'future', count);
      })
    }
     ...
     * @param {Function} successCallback
     */
    function getWaitingCount(queueName, successCallback) {
      client.llen([redisNamespace, queueName, 'list'].join('.'), function (err, count) {
        successCallback(queueName, 'queued', count);
      });
    }

    function getNextItem(queueName, successCallback) {
      client.zrangebyscore([
        [redisNamespace, queueName, 'index'].join('.'),
        '-inf',
        '+inf',
        'LIMIT',
        0,
        ...
        if (data[0]) {
          client.hgetall([redisNamespace, queueName, 'data'].join('.') + ':' + data[0], function (err, data) {
            if (data) {
      ...
      server.on('connection', function (sock) {
        ...
        sock.on('data', function (data) {
          if (data.toString().indexOf('COUNT') === 0) {
            ...
            const thingy = function (keys) {
              let totalCount = 0;
              let remaining = keys.length * 3;

              const callback = function (queue, type, singleCount) {
                if (queue) {
              ...
              for (var i = 0, len = keys.length; i < len; i++) {
                ...
                getNextItem(keys[i], function (queue, data) {
                  --remaining;
                  if (data) {
        ...
        sock.on('close', function () {
          if (backup) {
            ...
            client.rpush([redisNamespace, queue, 'list'].join('.'), backup.id);
